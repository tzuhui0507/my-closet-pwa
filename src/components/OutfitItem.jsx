...
      style={{
        position: 'absolute',
        // 👈 使用 translate3d 效能更好，且能避免 left/top 的排版閃爍
        transform: `translate3d(${item.x}px, ${item.y}px, 0) scale(${item.scale || 1})`,
        transformOrigin: 'center center',
        zIndex: active ? 100 : 1,
        touchAction: 'none', // 👈 必須：防止瀏覽器滾動攔截拖拽事件
...
        style={{
          display: 'block',
          width: '150px', // 👈 給予一個預設寬度確保圖片能顯示
          height: 'auto',
          pointerEvents: 'none', // 讓事件穿透到父層 div 處理
          outline: (active && !readOnly) ? '2px solid #007AFF' : 'none',
          outlineOffset: '4px',
          borderRadius: '8px',
